...
exports.handleAction = catchAsync(async (req, res, next) => {
    ...
    const tableName = entityTableMap[currentApproval.entity_type] || currentApproval.entity_type;

    if (!/^[a-zA-Z0-9_]+$/.test(tableName)) {
        return next(new AppError('Loại phê duyệt không hợp lệ.', 400));
    }
    ...
    if (action === 'Rejected') {
        ...
        await db.query(`UPDATE ${tableName} SET status = "Rejected" WHERE id = ?`, [currentApproval.entity_id]);
    ...
    if (nextApproverId) {
        ...
        await db.query(`UPDATE ${tableName} SET status = "Approved" WHERE id = ?`, [currentApproval.entity_id]);
