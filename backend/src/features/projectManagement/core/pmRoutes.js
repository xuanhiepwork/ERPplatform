...
const { validate, projectRules, taskRules } = require('../../../core/middlewares/validator');
...
// Global tasks list for Kanban board
router.get('/tasks', pmController.getAllTasks);

// Tạo task mới
router.post('/tasks', authMiddleware.requirePermission('manage_projects'), taskRules, validate, pmController.createTask);

// Cập nhật trạng thái Task (Kéo thả)
router.patch('/tasks/:taskId/status', authMiddleware.requirePermission('manage_projects'), pmController.updateTaskStatus);
