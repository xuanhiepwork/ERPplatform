...
const AuditLogger = require('../../../core/utils/auditLogger');
...
exports.generateMarketingContent = catchAsync(async (req, res, next) => {
    const { prompt, mode = 'content' } = req.body;

    if (!prompt || !String(prompt).trim()) {
        return next(new AppError('Please provide a marketing prompt.', 400));
    }
    ...
    const result = await model.generateContent(`
        You are a concise B2B marketing strategist.
        Mode: ${mode}
        Request: ${prompt}
        Return a practical draft with a headline, body copy, CTA, and a short SEO/quality score.
    `);
