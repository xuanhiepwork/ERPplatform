...
export function StrategyBoard() {
  ...
  const { data: pmTasks = [] } = useQuery({ queryKey: ['pm', 'tasks-strategy'], queryFn: taskApi.getAllTasks });

  const liveTasks: StrategyTask[] = (pmTasks || []).map((t: any) => ({
    id: `t-${t.id}`,
    title: t.title || t.name || `Task ${t.id}`,
    description: t.description || t.summary || '',
    column: t.status === 'done' ? 'completed' : (t.status === 'in-progress' ? 'in-progress' : (t.status === 'review' ? 'review' : 'planning')) as BoardColumn,
    priority: (t.priority || 'medium') as Priority,
    tags: (t.tags || ['Product']) as TeamTag[],
    assignees: t.assignee ? [{ name: t.assignee.name, avatar: t.assignee.avatar || '', initials: (t.assignee.name || '').split(' ').map((n: string) => n[0] || '').join('').slice(0, 2) }] : [],
    dueDate: t.due_date || t.dueDate || '',
    milestone: t.milestone || '',
    dependencies: (t.dependencies || []).map((d: any) => ({ taskId: d.taskId || d.id, type: d.type || 'relates-to' })),
    progress: t.progress || 0
  }));

  const getTasksByColumn = (columnId: BoardColumn) => {
    return (liveTasks && liveTasks.length > 0) ? liveTasks.filter(task => task.column === columnId) : tasks.filter(task => task.column === columnId);
  };
