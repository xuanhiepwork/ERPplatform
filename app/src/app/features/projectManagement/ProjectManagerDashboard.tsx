...
export function ProjectManagerDashboard() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-blue-50 p-6">
        ...
        <div className="bg-white rounded-lg border shadow-sm p-6">
          ...
          <div className="flex items-center gap-8">
            ...
            <div className="flex-1 flex justify-end">
              <Button className="bg-indigo-600 hover:bg-indigo-700" onClick={() => setShowGantt(true)}>
                View Gantt Chart
              </Button>
            </div>
          </div>
      ...
      {/* Gantt Modal (simple viewer) */}
      {showGantt && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
          <div className="absolute inset-0 bg-black/40" onClick={() => setShowGantt(false)} />
          <div className="relative bg-white rounded-lg shadow-lg w-[90%] max-w-4xl p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold">Gantt Data (Project preview)</h3>
              <Button variant="outline" size="sm" onClick={() => setShowGantt(false)}>Close</Button>
            </div>
            <GanttViewer projectId={1} />
          </div>
        </div>
      )}
    </div>
...
function GanttViewer({ projectId }: { projectId: number }) {
  const { data, isLoading, error } = useQuery({
    queryKey: ['pm', 'gantt', projectId],
    queryFn: async () => {
      const resp = await axiosClient.get(`/pm/projects/${projectId}/gantt`);
      return resp.data.data ?? resp.data;
    },
  });

  if (isLoading) return <div className="p-6">Loading Gantt data...</div>;
  if (error) return <div className="p-6 text-red-600">Failed to load Gantt data</div>;
