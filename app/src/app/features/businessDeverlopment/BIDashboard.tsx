...
type FunnelData = (typeof funnelData)[number];
type RevenueData = (typeof revenueData)[number];
...
export function BIDashboard() {
  ...
  const { data: partners = [] } = useQuery({ queryKey: ['bd-partners'], queryFn: bdApi.getPartners, staleTime: 1000 * 60 * 5 });
  const { data: deals = [] } = useQuery({ queryKey: ['bd-deals'], queryFn: bdApi.getPipelineDeals, staleTime: 1000 * 60 * 2 });

  // Khối useMemo này đã tự động tính toán và trả về đúng cấu trúc Object cho displayKPI
  const displayKPI = useMemo(() => {
    const computedProjectedRevenue = deals && Array.isArray(deals)
      ? deals.reduce((sum: number, d: any) => sum + (Number(d.expected_revenue || d.expectedRevenue || 0)), 0)
      : kpiData.projectedRevenue.value;

    const computedActivePartnerships = partners && Array.isArray(partners)
      ? partners.length
      : kpiData.activePartnerships.value;

    return {
      projectedRevenue: {
        value: computedProjectedRevenue,
        change: kpiData.projectedRevenue.change,
        trend: kpiData.projectedRevenue.trend,
        previousPeriod: kpiData.projectedRevenue.previousPeriod
      },
      conversionRate: kpiData.conversionRate,
      activePartnerships: {
        value: computedActivePartnerships,
        change: kpiData.activePartnerships.change,
        trend: kpiData.activePartnerships.trend,
        previousPeriod: kpiData.activePartnerships.previousPeriod
      }
    };
  }, [deals, partners]); // Tự động cập nhật displayKPI khi dữ liệu từ useQuery thay đổi
  ...
  const CustomTooltip = ({ active, payload, label }: TooltipProps<RevenueData>) => {
        ...
        <div className="bg-card border border-border rounded-lg shadow-lg p-3">
          ...
          {payload.map((entry, index: number) => (
            <p key={index} className="text-sm" style={{ color: entry.color }}>
              {/* SỬA TẠI ĐÂY: Bọc Number() xung quanh entry.value */}
  ...
  const FunnelTooltip = ({ active, payload }: TooltipProps<FunnelData>) => {
    if (active && payload && payload.length) {
          ...
          <CardContent className="p-6">
            <div className="flex items-start justify-between mb-4">
              <div>
                ...
                <h2 className="text-3xl">{formatFullCurrency(displayKPI.projectedRevenue.value)}</h2>
              </div>
            ...
            <div className="flex items-center gap-2">
              {displayKPI.projectedRevenue.trend === 'up' ? (
                <div className="flex items-center gap-1 text-green-600">
                  <ArrowUpRight className="h-4 w-4" />
                  <span className="text-sm font-medium">+{displayKPI.projectedRevenue.change}%</span>
                </div>
              ) : (
          ...
          <CardContent className="p-6">
            <div className="flex items-start justify-between mb-4">
              <div>
                ...
                <h2 className="text-3xl">{displayKPI.conversionRate.value}%</h2>
              </div>
            ...
            <div className="flex items-center gap-2">
              {displayKPI.conversionRate.trend === 'up' ? (
                <div className="flex items-center gap-1 text-green-600">
                  <ArrowUpRight className="h-4 w-4" />
                  <span className="text-sm font-medium">+{displayKPI.conversionRate.change}%</span>
                </div>
              ) : (
                <div className="flex items-center gap-1 text-red-600">
                  <ArrowDownRight className="h-4 w-4" />
          ...
          <CardContent className="p-6">
            <div className="flex items-start justify-between mb-4">
              <div>
                ...
                <h2 className="text-3xl">{displayKPI.activePartnerships.value}</h2>
              </div>
            ...
            <div className="flex items-center gap-2">
              {displayKPI.activePartnerships.trend === 'up' ? (
                <div className="flex items-center gap-1 text-green-600">
                  <ArrowUpRight className="h-4 w-4" />
                  <span className="text-sm font-medium">+{displayKPI.activePartnerships.change}</span>
                </div>
              ) : (
                <div className="flex items-center gap-1 text-red-600">
                  <ArrowDownRight className="h-4 w-4" />
