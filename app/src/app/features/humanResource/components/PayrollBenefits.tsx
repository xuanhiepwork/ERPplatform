...
function PayrollTable({ data, onRunPayroll }: { data: PayrollEntry[]; onRunPayroll?: () => void }) {
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-2">
            ...
            <Button className="bg-emerald-600 hover:bg-emerald-700" onClick={() => onRunPayroll && onRunPayroll()}>
...
export function PayrollBenefits() {
  ...
  // Admin payroll rows (start with mocks as fallback)
  const [payrollRows, setPayrollRows] = useState<PayrollEntry[]>(() => payrollData);

  // Fetch my payslips for the E-Payslip view (employee-facing)
  const { data: myPayslips = [] } = useQuery({ queryKey: ['hr-my-payslips'], queryFn: hrApi.getMyPayslips, staleTime: 1000 * 60 * 5 });

  // Mutation to run payroll (admin)
  const generateMutation = useMutation({
    mutationFn: ({ month, year }: { month: number; year: number }) => hrApi.generatePayrollDraft(month, year),
    onSuccess: (data: any[]) => {
      if (Array.isArray(data) && data.length > 0) {
        const mapped = data.map((p: any) => ({
          id: String(p.user_id || p.id || `${p.user_id}-${p.month}-${p.year}`),
          employeeName: p.full_name || 'Unknown',
          employeeId: p.user_id ? `EMP-${p.user_id}` : 'N/A',
          department: p.department || '',
          position: p.position || '',
          grossSalary: p.base_amount || 0,
          deductions: {
            socialInsurance: Math.round((p.deduction || 0) * 0.5),
            healthInsurance: Math.round((p.deduction || 0) * 0.25),
            personalIncomeTax: Math.round((p.deduction || 0) * 0.25),
          },
          bonuses: {
            performance: p.bonus?.performance || 0,
            attendance: p.bonus?.attendance || 0,
            other: p.bonus?.other || 0,
          },
          netPay: p.net_amount || 0,
          paymentStatus: (p.status || 'pending').toLowerCase(),
          paymentDate: p.pay_date || p.created_at || '',
          bankAccount: p.bank_account || '****0000',
        }));
  ...
  // Map first payslip for employee view if available
  const selectedPayslip = myPayslips && myPayslips.length > 0 ? myPayslips[0] : null;
  const payslipForView: PayslipData | null = selectedPayslip
    ? {
      id: String(selectedPayslip.id || `${selectedPayslip.user_id}-${selectedPayslip.month}-${selectedPayslip.year}`),
      employeeName: selectedPayslip.full_name || 'You',
      employeeId: selectedPayslip.user_id ? `EMP-${selectedPayslip.user_id}` : 'N/A',
      department: selectedPayslip.department || '',
      position: selectedPayslip.position || '',
      grossSalary: selectedPayslip.base_amount || 0,
      deductions: {
        socialInsurance: Math.round((selectedPayslip.deduction || 0) * 0.5),
        healthInsurance: Math.round((selectedPayslip.deduction || 0) * 0.25),
        personalIncomeTax: Math.round((selectedPayslip.deduction || 0) * 0.25),
      },
      bonuses: {
        performance: selectedPayslip.bonus?.performance || selectedPayslip.bonus || 0,
        attendance: selectedPayslip.bonus?.attendance || 0,
        other: selectedPayslip.bonus?.other || 0,
      },
      netPay: selectedPayslip.net_amount || 0,
      paymentStatus: (selectedPayslip.status || 'pending').toLowerCase(),
      paymentDate: selectedPayslip.created_at || '',
      payPeriod: `${selectedPayslip.month || ''}/${selectedPayslip.year || ''}`,
      payDate: selectedPayslip.created_at || '',
      workingDays: selectedPayslip.working_days || selectedPayslip.total_paid_days || 0,
      totalWorkingDays: 22,
      overtimeHours: 0,
      overtimePay: 0,
      bankAccount: selectedPayslip.bank_account || '****0000',
    }
    : null;
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 overflow-auto">
        {activeTab === 'payroll' && <PayrollTable data={payrollRows} onRunPayroll={runPayroll} />}
        {activeTab === 'payslip' && (payslipForView ? <EPayslipView data={payslipForView} /> : <Card className="p-6">No payslip found for your account.</Card>)}
      </div>
    </div>
  );
}
