...
// Timesheet and leave data will be fetched from the backend via `hrApi` inside the component.
...
// Leave requests are fetched from the server for the logged-in user.
...
export function TimeAttendance() {
  ...
  const { data: timesheetData = [], isLoading: isTimesheetLoading } = useQuery({
    queryKey: ['attendance-my-timesheet'],
    queryFn: hrApi.getMyTimesheet,
    staleTime: 1000 * 60,
  });
  ...
  const { data: me } = useQuery({ queryKey: ['hr-profile'], queryFn: hrApi.getEmployeeProfile, staleTime: 1000 * 60 * 5 });

  // Normalize attendance rows returned by backend into the TimesheetEntry shape
  const mappedTimesheet: TimesheetEntry[] = (timesheetData || []).map((r: any, idx: number) => {
    const checkIn = r.check_in ? new Date(r.check_in) : null;
    const checkOut = r.check_out ? new Date(r.check_out) : null;
    const totalHours = checkIn && checkOut ? Math.max(0, (checkOut.getTime() - checkIn.getTime()) / (1000 * 60 * 60)) : 0;

    return {
      id: String(r.id || idx),
      employeeName: r.full_name || r.employee_name || (me && me.name) || 'Me',
      employeeId: r.employee_id || r.user_id || (me && me.employeeId) || 'N/A',
      department: r.department || (me && me.department) || '',
      date: checkIn ? checkIn.toLocaleDateString() : '',
      clockIn: checkIn ? checkIn.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '--',
      clockOut: checkOut ? checkOut.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '--',
      totalHours: Number(totalHours.toFixed(2)),
      status: (r.status && (['on-time', 'late', 'absent'].includes(r.status) ? r.status : 'on-time')) as any,
      expectedIn: r.expected_in || '09:00 AM',
      expectedOut: r.expected_out || '05:30 PM'
    };
  });

  const mappedLeaves: LeaveRequest[] = (leaveRequestsData || []).map((r: any, idx: number) => ({
    id: String(r.id || idx),
    employeeName: me?.name || 'You',
    employeeId: me?.employeeId || 'EMP-000',
    department: me?.department || '',
    leaveType: (r.leave_type || 'Annual').toLowerCase() as any,
    startDate: r.start_date || r.startDate || '',
    endDate: r.end_date || r.endDate || '',
    days: r.days || Math.max(1, Math.ceil((new Date(r.end_date).getTime() - new Date(r.start_date).getTime()) / (1000 * 60 * 60 * 24) + 1)),
    reason: r.reason || '',
    status: (r.status || 'pending') as any,
    submittedDate: r.created_at || r.submittedDate || '',
    remainingLeave: r.remaining_leave || { annual: 12, sick: 5 }
  }));
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 overflow-auto">
        {activeTab === 'timesheet' && <TimesheetTable data={mappedTimesheet} />}
        {activeTab === 'shifts' && <ShiftScheduler shifts={shiftAssignments} />}
        {activeTab === 'leave' && <LeaveManagement requests={mappedLeaves} />}
      </div>
    </div>
  );
}
