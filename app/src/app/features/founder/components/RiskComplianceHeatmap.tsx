...
export function RiskComplianceHeatmap() {
  ...
  const { data: contractsData } = useQuery({ queryKey: ['strategic-contracts'], queryFn: contractApi.getAllContracts, staleTime: 1000 * 60 * 5 });
  const strategic = contractsData ?? strategicContracts;

  const { data: newsfeedData } = useQuery({
    queryKey: ['comms-newsfeed'], queryFn: async () => {
      try {
        const res = await axiosClient.get('/comms/newsfeed');
  ...
  const audit = (newsfeedData && Array.isArray(newsfeedData) && newsfeedData.length > 0)
    ? newsfeedData.map((n: any) => ({ timestamp: n.created_at || n.createdAt, user: n.author_name || 'System', action: n.content || n.message, level: 'medium' }))
    : auditTrail;
