...
import { DollarSign, TrendingUp, ChevronDown, Shield, Sliders } from 'lucide-react';
...
export function StrategicFinance() {
  ...
  const { data } = useQuery({ queryKey: ['executive-metrics'], queryFn: founderApi.getExecutiveMetrics, staleTime: 1000 * 60 * 2 });
  const finance = data?.finance ?? null;

  const plSummary = {
    revenue: Number(finance?.summary?.totalRevenue) || 12400000,
    cogs: Number(finance?.summary?.totalExpense) || 4960000,
    grossProfit: (Number(finance?.summary?.totalRevenue) || 12400000) - (Number(finance?.summary?.totalExpense) || 4960000),
    operating: Number(finance?.summary?.totalExpense) || 5580000,
    netProfit: Number(finance?.summary?.netProfit) || 1860000,
  };

  const plDetails = [
    {
      category: 'Revenue', items: [
        { name: 'Enterprise Sales', amount: Math.round(plSummary.revenue * 0.65) },
        { name: 'SMB Sales', amount: Math.round(plSummary.revenue * 0.25) },
        { name: 'Professional Services', amount: Math.round(plSummary.revenue * 0.1) },
      ]
    },
    {
      category: 'Cost of Goods Sold', items: [
        { name: 'Cloud Infrastructure', amount: Math.round(plSummary.cogs * 0.5) },
        { name: 'Support & Maintenance', amount: Math.round(plSummary.cogs * 0.3) },
        { name: 'Third-party Licenses', amount: Math.round(plSummary.cogs * 0.2) },
      ]
    },
    {
      category: 'Operating Expenses', items: [
        { name: 'Salaries & Benefits', amount: Math.round(plSummary.operating * 0.6) },
        { name: 'Marketing & Sales', amount: Math.round(plSummary.operating * 0.25) },
        { name: 'R&D', amount: Math.round(plSummary.operating * 0.1) },
        { name: 'General & Admin', amount: Math.round(plSummary.operating * 0.05) },
      ]
    },
  ];

  const cashflowForecast = (finance?.chartData && Array.isArray(finance.chartData) && finance.chartData.length > 0)
    ? finance.chartData.map((d: any) => ({ month: d.date, cashflow: Math.round((d.revenue - d.expense) / 1000), cumulative: 0 }))
    : [
  ...
  // compute cumulative if backend didn't provide
  let cum = 0;
  cashflowForecast.forEach((r: any) => { cum += r.cashflow; r.cumulative = cum; });

  const maxCashflow = Math.max(...cashflowForecast.map((d: any) => d.cashflow || 0));
  ...
  return (
    <div className="min-h-screen bg-slate-950 p-8">
      <div className="max-w-[1800px] mx-auto space-y-8">
        ...
        <div className="bg-gradient-to-br from-slate-900 to-slate-950 rounded-2xl border border-slate-800 p-6">
          <div className="flex items-center justify-between mb-6">
            {/* ĐÃ FIX DẤU > THÀNH HTML ENTITY &gt; Ở ĐÂY */}
