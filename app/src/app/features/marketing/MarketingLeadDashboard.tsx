...
export function MarketingLeadDashboard() {
  ...
  const normalizedCampaigns = (campaignsData || []).map((c: any) => ({
    ...
    ctr: Number(c.ctr || c.click_through_rate || 0),
    cpc: Number(c.cpc || c.cost_per_click || 0),
    conversions: Number(c.conversions || c.conversion_count || 0),
    budget: Number(c.budget || c.budget_percent || 0),
  }));

  const campaigns = normalizedCampaigns.length ? normalizedCampaigns : campaignsMock;
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-4 gap-4">
          <div className="bg-white rounded-lg border shadow-sm p-6">
            ...
            <div className="text-3xl font-bold text-gray-900">{perfData?.impressions ?? '2.4M'}</div>
          ...
          <div className="bg-white rounded-lg border shadow-sm p-6">
            ...
            <div className="text-3xl font-bold text-gray-900">{perfData?.ctr ?? '4.2%'}</div>
            <div className="flex items-center gap-1 text-xs text-green-600 mt-1">
              <TrendingUp className="h-3 w-3" />
              +0.8% improvement
            </div>
          ...
          <div className="bg-white rounded-lg border shadow-sm p-6">
            ...
            <div className="text-3xl font-bold text-gray-900">{perfData?.conversion_rate ?? '18.5%'}</div>
          ...
          <div className="bg-white rounded-lg border shadow-sm p-6">
            ...
            <div className="text-3xl font-bold text-gray-900">{perfData?.roi ?? '342%'}</div>
            <div className="flex items-center gap-1 text-xs text-green-600 mt-1">
              <TrendingUp className="h-3 w-3" />
              +18% vs target
            </div>
