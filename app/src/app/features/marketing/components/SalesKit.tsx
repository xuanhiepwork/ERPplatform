...
export function SalesKit() {
  ...
  const handleDownload = (asset: Asset) => {
    const blob = new Blob([`${asset.title}\n${asset.description}\nVersion: ${asset.version}`], { type: 'text/plain;charset=utf-8' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = `${asset.title.replace(/[^a-z0-9]+/gi, '-').toLowerCase()}-${asset.version}.txt`;
    link.click();
  ...
  const handleShare = async (asset: Asset) => {
    const text = `${asset.title} (${asset.version}) - ${asset.description}`;
    if (navigator.share) {
      await navigator.share({ title: asset.title, text }).catch(() => undefined);
      return;
    }
