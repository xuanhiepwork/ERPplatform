...
export function CreativeBoard() {
  ...
  const { data: campaignsData = [] } = useQuery({ queryKey: ['marketing-campaigns'], queryFn: marketingApi.getCampaigns, staleTime: 1000 * 60 * 5 });
  const updateStatusMutation = useMutation({
    mutationFn: (ticketId: string) => marketingApi.updateCampaignStatus(ticketId, 'Scheduled'),
    onSuccess: () => {
      toast.success('Request moved into production');
      queryClient.invalidateQueries({ queryKey: ['marketing-campaigns'] });
    },
    onError: () => toast.error('Could not update request status'),
  });

  const normalizedCampaignTickets: CreativeTicket[] = (campaignsData || []).map((c: any, idx: number) => ({
    id: String(c.id || c.campaign_id || `camp-${idx}`),
    title: c.title || c.name || 'Untitled Campaign',
    type: (c.channel && c.channel.includes('video')) ? 'video' : (c.channel && c.channel.includes('email')) ? 'email' : 'social-media',
    priority: (c.priority || 'medium') as Priority,
    status: (c.status && (['Draft', 'Scheduled'].includes(c.status) ? 'briefing' : c.status === 'Published' ? 'ready' : 'inbox')) as TicketStatus,
    requestedBy: {
      name: c.owner_name || c.requester_name || 'Marketing',
      department: 'Marketing',
      avatar: c.owner_avatar || '',
      initials: (c.owner_name || 'M').split(' ').map((s: string) => s[0]).slice(0, 2).join('').toUpperCase()
    },
    assignedTo: {},
    dueDate: c.publish_date || c.due_date || '',
    subTasks: [],
    description: c.description || c.summary || '',
    requestDate: c.created_at || c.requestDate || ''
  }));

  const ticketsList = normalizedCampaignTickets.length ? normalizedCampaignTickets : tickets;

  const inboxTickets = ticketsList.filter(t => t.status === 'inbox');
  const kanbanTickets = ticketsList.filter(t => t.status !== 'inbox');
  ...
  const moveToKanban = (ticketId: string) => {
    if (ticketId.startsWith('REQ-')) {
      toast.info('This sample request is not stored in the backend yet');
      return;
    }
    updateStatusMutation.mutate(ticketId);
  };
