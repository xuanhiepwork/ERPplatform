...
export function DAMPortal() {
  ...
  const { data: assetsData = [] } = useQuery({ queryKey: ['marketing-assets', searchQuery, selectedCategory], queryFn: async () => marketingApi.getAssets(searchQuery), staleTime: 1000 * 60 * 5 });

  const mapAssetRow = (a: any): Asset => {
    const fileName = a.file_name || a.name || 'asset';
    const ext = fileName.split('.').pop() || '';
    const fileType = (a.file_type || '').toLowerCase();
    const type = fileType.includes('image') ? 'image' : fileType.includes('video') ? 'video' : 'document';
    const thumbnail = a.file_url || a.thumbnail || '';
    const size = a.file_size ? `${Math.round((Number(a.file_size) / 1024 / 1024) * 10) / 10} MB` : '—';
    const uploadDate = a.created_at || a.uploadDate || '';
    const tags = a.tags ? (typeof a.tags === 'string' ? a.tags.split(',').map((t: string) => t.trim()) : a.tags) : [];
    const versions = a.versions || (a.version ? [{ version: a.version, date: uploadDate, uploadedBy: a.uploaded_by_name || '' }] : []);
    const formats = ext ? [ext.toUpperCase()] : [];
    return {
      id: String(a.id || a.asset_id || Math.random()),
      name: fileName,
      type,
      thumbnail,
      size,
  ...
  const assets = (assetsData && assetsData.length) ? assetsData.map(mapAssetRow) : mockAssets;
